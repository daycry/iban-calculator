...
final class CommandsTest extends CIUnitTestCase
{
     ...
     * `CentralBankOfMaltaImporter` (MT). So with no `--country`/`--source`
     * selection, `iban:update` now lists all seventeen alongside the v1.0
    ...
    public function testUpdatePrintsLicenseNoticesAndListsTheBundledImportersAndExitsSuccess(): void
    {
        ...
        self::assertStringContainsString('Registered importers: 17', $output);
    ...
    {
        // 'FR' matches none of the 17 bundled importers
        // (AT/DE/CH/NL/ES/CZ/GR/SI/SK/BG/MD/PL/AZ/BE/HR/LU/MT), so this stays
        // the graceful "no match" branch -- and, crucially, never reaches
        // the network/file fetch.
